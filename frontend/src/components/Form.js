...
const Form = ({ image, setImage, isPending, setIsPending, url, setUrl, setError }) => {
  ...
  return (
    <div className="flex flex-col h-[50vh] drop-shadow-lg p-5 justify-between bg-white w-4/5 md:w-2/6 sm:w-4/6 rounded-md">
      ...
      <div
        {...getRootProps({
          className: 'md:h-52 sm:h-44 h-auto bg-light-grey border-2 border-light-blue border-dashed rounded-md',
        })}
      >
      ...
      <button onClick={open} className="bg-blue text-white font-normal p-1 rounded-lg w-auto mx-auto px-4 py-2 text-md">
        Choose a file
      </button>
    </div>
  );
